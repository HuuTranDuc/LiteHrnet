...
for i in range(len(output)):
    ...
    all_aps.append((ap, output[i]["image_paths"], det_kpts_all, gt_kpts_all))



sorted_aps = sorted(all_aps, key=lambda x: x[0])
for i in range(len(output)):
    min_ap, path_img, keypoints_output_draw, keypoints_ground_truth = sorted_aps[i]
    print(f"AP image {i}: {min_ap:.4f}")
    print(path_img[0])
    ...
    keypoints_output_draw = np.array(keypoints_output_draw).astype(int)
    keypoints_output_draw = keypoints_output_draw[:,:2]
    img_draw = draw_keypoints(image_copy, keypoints_ground_truth, lm_color=(0,255,0), edge_color=(0,255,0))            # Xanh lá là label
